...
> **Vivado version:** _record the version you build with here_ — e.g. `2023.2`. Tcl
> commands and default synthesis strategies drift between releases; pinning the version
> is the difference between "it builds" and "it built once, for one person".
...
Being blunt about this up front saves a long confusing synthesis run.

**`mxu` at 8×8 is fine.** Roughly 20k flip-flops, about half the device — dominated not
by the 8×8 array (2k FFs of partial sums) but by `resbuf[0:63][0:7]` of int32, which is
16k FFs on its own. If area gets tight, `TOK_W` is the knob.
...
Two known functional gaps on hardware, both matching current simulation behaviour rather
than being introduced by this flow:

- `GELU_INIT` / `EXP_INIT` are empty, so the VPU's activation LUTs are all zeros. Ops
  that use them return zeros until those `.hex` tables are generated from `model/` and
  passed into the wrapper.
