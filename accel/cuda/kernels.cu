...
__global__ flash_attention_mha(const float* Q, const float* K, const float* V, float* atn_scores, float* output, int b, int t, int s, int head_dim, int q_heads, int kv_heads) {
    __shared__ 
}