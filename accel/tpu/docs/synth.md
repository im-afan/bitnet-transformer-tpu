...
**It fits, and it routes with timing met.** Measured, post-route, at the `boards/cmod_a7`
geometry (8×8, `addr_w=16`) on the `xc7a35t-cpg236-1`:
...
timing was never going to be the constraint; **LUTs are**, at 92%.
...
The VPU's activation LUTs *are* loaded: `GELU_INIT` / `EXP_INIT` in `cmod_a7_top.sv`
point at `rtl/luts/{gelu,exp}_lut.hex`, generated by `python accel/tpulang/luts.py`.
Those paths are relative to `accel/tpu`, which `read_design` cds into before reading the
RTL so they resolve identically no matter where Vivado was launched — if you move the
build script, keep that `cd`, because a `$readmemh` that misses only *warns* and leaves
the ROM zero-filled, which looks exactly like an op that computes zero.
