...
torch::Tensor linear_forward(torch::Tensor embedding, torch::Tensor W,
  ...
  linear(embedding.data_ptr<float>(), W.data_ptr<float>(), B.data_ptr<float>(),
         output.data_ptr<float>(), embedding_dim, batch_size);

  return output;
}

torch::Tensor attention_softmax(torch::Tensor KQ_row, float scale) {
  CHECK_INPUT(KQ_row);

  int batch_size = KQ_row.size(0);
  int n_tokens = KQ_row.size(1);

  auto output = torch::empty({batch_size, n_tokens}, KQ_row.options());
...
//       embedding.data_ptr<float>(), K_cache.data_ptr<float>(),
//       V_cache.data_ptr<float>(), W_K.data_ptr<float>(),
//       W_Q.data_ptr<float>(), W_V.data_ptr<float>(), W_O.data_ptr<float>(),
//       B_K.data_ptr<float>(), B_Q.data_ptr<float>(), B_V.data_ptr<float>(),
//       B_O.data_ptr<float>(), output.data_ptr<float>(), n_tokens, batch_size,
//       embedding_dim, n_heads, max_tokens);
